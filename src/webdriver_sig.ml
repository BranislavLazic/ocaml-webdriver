module type HTTP_CLIENT = sig
  ...
  (** [post url body] returns the body of the response. The [Content-Type]
...
module type S = sig
  ...
  (** WebDriver is a {{:https://www.w3.org/TR/webdriver/} W3C specification} to
      ...
      enabled, on as many browsers and operating systems you can get your hands
      on. *)

  type json = Yojson.Safe.t

  type 'a io
  (** The client I/O monad, within which communication happens with the
      WebDriver server. *)

  (** {1 Commands} *)

  type session
  (** A connection to the WebDriver server. A session statefully holds the
      active windows, tabs, cookies, and other state required by the browser. *)

  type 'a cmd = session:session -> 'a io
  (** Every browser command takes place within a [session]. *)

  (** Since the [~session] parameter is generally constant, this module provides
      a reader monad to sequence multiple commands within the same session. You
      can either pass explicitly the [~session] argument or open this module. *)
  module Infix : sig
    val lift : 'a io -> 'a cmd
  ...
  module Error : sig
    (** See the
        {{:https://www.w3.org/TR/webdriver/#errors} WebDriver specification} for
        an explanation of the source of an error: *)
    ...
    *)
    ...
    val catch :
      (unit -> 'a cmd) -> ?errors:error list -> (t -> 'a cmd) -> 'a cmd
    (** [catch (fun () -> cmd) ?errors (fun err -> catch)] runs [cmd], catching
        any WebDriver exceptions from [?errors] (or all if unspecified), then
        runs [catch] if necessary with the error [err] as an argument.
  ...
  (** {1 Sessions}
      ...
      - The [host] is a url where the WebDriver server can be accessed, for
  ...
  module Capabilities : sig
    (** See the
        {{:https://www.w3.org/TR/webdriver/#capabilities} WebDriver
         specification} and the documentation of the chosen browser for more
        informations. *)
    ...
    (** Same as [chrome], but runs in the background without a graphical window:
        {[
          { "capabilities":
              { "firstMatch":
                  [{ "browser_name": "chrome",
    ...
    (** Same as [firefox], but runs in the background without a graphical
        window:
        {[
          { "capabilities":
              { "firstMatch":
  ...
  module Session : sig
    ...
    (** [make ~host cap] creates a new session on [host] with the required
        [cap]abilities. It returns the created [session] and a [json] describing
        the available capabilities of the session. *)
  ...
  (** [run ~host capabilities cmd] is a helper function to create a new session
      on [host] with the required [capabilities], execute the [cmd] within that
      session, and finally ensure that the session is deleted on termination of
      [cmd] (from its natural death or an exception.) *)

  (** Configure the timeouts for page loads, script execution and the implicit
      wait when searching for elements on a page. *)
  module Timeouts : sig
    (** - The [script] timeout specifies when to interrupt a script that is
          being evaluated.

        - The [page_load] limits the time it takes to navigate and load an url.

        - The [implicit_wait] adds a delay to every {! find_first} and
          {! find_all}, to leave time for the searched element to be created. *)

    type t = {
      script : [ `Never | `After of int ];
          (** default is [`After 30_000] ms (= 3s) *)
      page_load : int;  (** default is [300_000] ms (= 300s) *)
      implicit_wait : int;  (** default is [0] ms *)
    }
    ...
    val set :
      ...
      unit ->
      unit cmd
    (** [set ?script ?page_load ?implicit_wait ()] updates the configuration for
        the provided fields. *)
  end

  (** Even though the browser attempts to complete most operations before giving
      back control, some commands might trigger too soon and raise an error. The
      recommended strategy is to sleep and retry the operation repeatedly until
      it succeeds. *)
  module Wait : sig
    (** The default parameters are:
        - [?sleep = 50ms] is the amount of time to {! sleep} before retrying the
          command.
        - [?max = 3000ms] is the maximum total time spend waiting in between
    ...
    val retry :
      ?max:int -> ?sleep:int -> ?errors:Error.error list -> 'a cmd -> 'a cmd
    ...
    val until :
      ?max:int -> ?sleep:int -> ?errors:Error.error list -> bool cmd -> unit cmd
    (** [until condition] behaves like [retry condition], but the predicate must
        also be satisfied at the end.

        Raises [`unspecified "condition"] if the [max] time is reached and the
  ...
  module Cookie : sig
    ...
    (** A helper function to create a new cookie with some optional parameters.
        The resulting cookie will not be set in the browser ands needs to be
        {! add}ed. *)
  ...
  (** [find_first `using "query"] returns the first element that matches the
      [query] (interpreted with [using]). The element is searched inside the
      current frame of the current window, and if a [?from] parent element is
      provided, the search takes place inside it.
  ...
  (** [find_all `using "query"] behaves like {! find_first}, but returns a list
      of all elements matching the [query]. *)
  ...
  (** [attribute e attr] returns the value of the HTML attribute [attr] of the
      element [e]. *)

  val property : elt -> string -> string option cmd
  (** [property e prop] returns [Some] value of the DOM property [prop] of the
      element [e], or [None] if undefined. *)
  ...
  (** [css e prop] returns the computed value of the css property [prop] for the
      element [e]. *)

  val aria_role : elt -> string cmd
  (** [aria_role e] returns the accessibility role of the element [e]. See
      {{:https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles}
       ARIA Roles on MDN} *)

  val aria_label : elt -> json cmd
  (** [aria_role e] returns the accessibility label of the element [e]. See
  ...
  (** [perform actions] executes the sequence of actions for each input source.
      The actions are synchronized vertically, such that:

      {[
        perform
          [
            mouse [ `down button0; `pause; `up button0 ];
      ...
      The pressed keys and buttons stay pressed at the end of the interaction,
  ...
  (** An inoperative device, that can be used to time the duration of each
      vertical frame. *)
  ...
  *)
  ...
  type move
  (** A pointer movement to a new location, taking some [duration] of time (in
      milliseconds). The default [duration] is a teleportation in [0]ms. *)

  val absolute : ?duration:int -> int * int -> move
  (** [absolute (x, y)] moves the pointer to the position [(x, y)] measured from
      the top left of the document. *)

  val relative : ?duration:int -> int * int -> move
  (** [relative (dx, dy)] moves the pointer by [(dx, dy)] from its current
      location. *)

  val center : ?duration:int -> ?offset:int * int -> elt -> move
  (** [center ~offset:(dx, dy) elt] moves the pointer to the center of the
      element [elt] offsetted by [offset]. The default [offset] is [(0, 0)]. *)
  ...
  (** [touch actions] describes the interactions of a touch finger device. If
      multiple touch devices are used in the same {! perform}, they must have
      different names. *)
  ...
  (** [scroll_absolute ~x ~y ()] resets the scrollbar such that the position
      [x, y] falls into view, as measured from the top of the page. The default
      value of [x] and [y] is 0. *)

  val scroll_to : ?duration:int -> ?dx:int -> ?dy:int -> elt -> scroll
  (** [scroll_to ~dx ~dy elt] resets the scrollbar such that the center of the
      element [elt], offsetted by [(dx, dy)], is inside the view. The default
      offset of [dx] and [dy] is 0. *)

  type wheel = [ pause | `scroll of scroll  (** scroll to a position *) ]
  ...
  *)
  ...
  module Window : sig
    ...
    (** [set_rect r] attempts to resize the [current] window to match the
        rectangle [r]. *)
  ...
  (** [excute "js"] runs the [js] on the current page, returning its result in
      [json]. *)
  ...
  (** [excute_async "js"] runs the [js] asynchronously on the current page.
      ...
      called, and returns its parameter as json. This can be used to block until
      some component has initialized:
