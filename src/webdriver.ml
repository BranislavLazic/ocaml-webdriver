...
module Make (Client : HTTP_CLIENT) = struct
  ...
  module Error = struct
    ...
    let to_string e =
      Printf.sprintf "{ error = %S ; message = %S ; data = %s ; _ }"
        (string_of_kind e.error) e.message
    ...
    let catch m ?(errors = []) f =
     fun ~session ->
      Client.catch
        (fun () -> m () ~session)
        (function
          | Webdriver e when is_match e errors -> f e ~session
          | exn -> Client.fail exn)
  ...
  module J = struct
    ...
    let bool ?default json =
      match (json, default) with
      | `Bool b, _ -> b
  ...
  let get ~session path = client_value (Client.get (session ^ path))

  let post_raw ~session path body =
    client_value (Client.post (session ^ path) body)

  let post ~session path json =
    let body = match json with `Null -> "{}" | json -> Json.to_string json in
    post_raw ~session path body

  let delete ~session path = client_value (Client.delete (session ^ path))
  ...
  let run ~host capabilities t =
    ...
    match result with Ok x -> Client.return x | Error e -> Client.fail e
  ...
  module Window = struct
    ...
    let string_of_hint = function `tab -> "tab" | `window -> "window"
  ...
  type using = [ `css | `link_text | `partial_link_text | `tag_name | `xpath ]
  ...
  let strategy using selector =
    `Assoc
      [
        ("using", `String (string_of_using using)); ("value", `String selector);
      ]
  ...
  let expect_id = function
    | `Assoc [ (key, `String id) ] ->
        ...
        id
    | json -> Error.protocol_fail "expected web element identifier" json

  let from id = "/element/" ^ id
  let from_opt = function None -> "" | Some id -> from id
  ...
  let attribute elt attr = J.string |<< get (from elt ^ "/attribute/" ^ attr)

  let property elt prop =
    J.string_option |<< get (from elt ^ "/property/" ^ prop)

  let css elt prop = J.string |<< get (from elt ^ "/css/" ^ prop)
  ...
  let switch_to_parent_frame = J.unit |<< post "/frame/parent" `Null
  let screenshot ?elt () = J.base64 |<< get (from_opt elt ^ "/screenshot")

  type pause = [ `noop | `pause of int ]
  type key = [ pause | `down of string | `up of string ]
  type button = int
  ...
  let relative ?(duration = 0) (x, y) =
    { move_duration = duration; move_origin = `pointer; move_x = x; move_y = y }
  ...
  type pointer =
    [ pause | `cancel | `down of button | `up of button | `move of move ]
  ...
  type wheel = [ pause | `scroll of scroll ]
  ...
  let json_of_key_tick = function
    ...
    | `up key ->
        `Assoc
          [
            ("type", str "keyUp"); ("value", `Stringlit (Key.escape_unicode key));
          ]
  ...
  let string_of_origin = function `viewport -> "viewport" | `elt id -> id
  ...
  let assoc_of_kind : type a. a kind -> a list -> (string * Yojson.Raw.t) list =
   fun kind lst ->
    match kind with
    | Null ->
        [
    ...
    | Key ->
        [
          ("type", str "key"); ("actions", `List (List.map json_of_key_tick lst));
        ]
    | Wheel ->
        [
          ("type", str "wheel");
  ...
  let sleep duration = perform [ none [ `pause duration ] ]
  let key_modifiers = [ Key.control; Key.shift; Key.alt; Key.meta ]
  let is_modifier k = List.mem k key_modifiers

  let typing keys =
    let rec go acc i =
      if i >= String.length keys then acc
      else if Str.string_match Key.re_unicode keys i then
        let key = String.sub keys i 6 in
        if is_modifier key then `down key :: go (`up key :: acc) (i + 6)
        else (`down key :: `up key :: acc) @ go [] (i + 6)
      else
        let key = String.sub keys i 1 in
  ...
  module Cookie = struct
    ...
    let make ?(path = "/") ?domain ?(secure = false) ?(http_only = false)
        ?(expiry = 999) ?(same_site = `Undef) ~name ~value () =
      { name; value; path; domain; secure; http_only; expiry; same_site }
    ...
    let nullable_string = function None -> `Null | Some str -> `String str
  ...
  module Timeouts = struct
    ...
    let set ?script ?page_load ?implicit_wait () =
      ...
      in
      let page_load =
        match page_load with None -> [] | Some ms -> [ ("pageLoad", `Int ms) ]
      in
      let implicit_wait =
        match implicit_wait with
        | None -> []
